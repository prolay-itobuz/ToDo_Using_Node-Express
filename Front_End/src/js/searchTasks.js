...
searchForm.addEventListener("submit", async function (event) {
    ...
    if (!searchBox.value) {
        toastSection.innerHTML = taskTemplates.errorToast('Please Enter Details to Search.');
        setTimeout(() => {
            toastSection.innerHTML = ''
        }, 4000);
    }
    else if (searchBox.value.length < 3) {
        toastSection.innerHTML = taskTemplates.errorToast('Please Enter min 3 character to Search');
        setTimeout(() => {
            toastSection.innerHTML = ''
        }, 4000);
    }
        ...
        if (data) {
            for (let i = 0; i < data.data.length; i++) {
                if (data.data[i].isImportant) {
                    searchTasks_section.innerHTML += taskTemplates.showImportant(data.data, i)
                }
                else {
                    searchTasks_section.innerHTML += taskTemplates.showActive(data.data, i)
                }
            }
