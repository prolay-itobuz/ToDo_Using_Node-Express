...
const noTask_complete = document.getElementById("noTask_complete")
const searchBox = document.getElementById("searchInput")
const searchTasks_section = document.getElementById("searchTasks")

function emptyTask_active() {
...
async function init() {
    ...
    if (tasks.length === 0) {
        emptyTask_active();
        noTask_complete.classList.remove("d-none")
        searchBox.setAttribute('disabled', '')
        searchTasks_section.classList.add("d-none")
    }

    for (let i = 0; i < tasks.length; i++) {
        if (tasks[i].is_completed === false) {
            if (tasks[i].is_important === true) {
                importantTasks_section.innerHTML += `<div class="task-manager__task-card task-manager__task-card--important">
                        ...
                        <div class="task-manager__task-actions">
                            ...
                            <button class="btn task-manager__action-btn task-manager__action-btn--important" onclick="changeImportance(${tasks[i].id})">
            ...
            completeTasks_section.innerHTML += `<div class="task-manager__task-card task-manager__task-card--completed">
                    <div class="task-manager__task-header">
                        <div>
                            <h4 class="task-manager__task-title task-manager__task-title--completed">${tasks[i].title}</h4>
                        </div>
                    </div>
...
deleteForm.addEventListener("submit", async function (event) {
    ...
    console.log(temp)
    await API.deleteTodo(parseInt(temp))
    window.location.reload()
});
...
searchForm.addEventListener("submit", async function (event) {
    ...
    if (searchBox.value) {
        ...
        const tasks = todos.filter(todo =>
            !todo.is_completed &&
            (todo.title.toLowerCase().includes(query) || todo.tags.toLowerCase().includes(query))
        );
        console.log(tasks)
